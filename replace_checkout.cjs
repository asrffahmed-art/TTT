...
const replacement = `      {/* Payment Checkout Modal */}
      {selectedPlanForPay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a0a0a] overflow-y-auto p-4 sm:p-6 font-['Be_Vietnam_Pro'] text-[#e5e2e1]" dir="rtl">
          {/* Background Effects */}
          ...
          <main className="w-full max-w-[1200px] flex flex-col md:flex-row overflow-hidden relative z-10 animate-in fade-in zoom-in-95 my-auto shadow-[0_30px_60px_-15px_rgba(0,0,0,0.9),0_0_40px_rgba(230,57,70,0.03)] rounded-3xl" style={{ background: 'rgba(15, 15, 15, 0.6)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
            {/* Close Button Mobile - Absolute on top */}
            <button 
              onClick={() => {
                setSelectedPlanForPay(null);
                setIsProcessingPayment(false);
              }}
              className="absolute top-6 left-6 z-50 p-2 bg-white/5 hover:bg-white/10 text-white rounded-full md:hidden transition-colors"
            >
              <X className="w-5 h-5" />
            </button>

            {/* Right Panel - Checkout Form (RTL means it's on the right visually) */}
              ...
              <div className="flex-1">
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      <CreditCard className={\`w-8 h-8 \${paymentMethod === 'paymob' || paymentMethod === 'card' ? 'text-[#e63946]' : 'text-white/60'}\`} />
                      <span className={\`font-semibold text-xs text-center \${paymentMethod === 'paymob' || paymentMethod === 'card' ? 'text-white' : 'text-white/60'}\`}>بطاقة ائتمان</span>
                    </div>
                    ...
                    >
                      ...
                      <span className={\`font-semibold text-xs text-center \${paymentMethod === 'paypal' ? 'text-white' : 'text-white/60'}\`}>PayPal</span>
                    </div>
                  </div>
                </div>
              ...
              {(!paymentMethod || paymentMethod === 'paymob' || paymentMethod === 'card') && (
                <div className="mt-12 flex gap-4 w-full">
                  ...
                  <button 
                    onClick={() => {
                      if (paymentMethod === 'paymob' || paymentMethod === 'card') {
                        // Dispatch event to trigger Paymob SDK
                  ...
                  >
                    ...
                    <div className="absolute inset-0 rounded-xl" style={{ animation: 'pulseGlow 2s infinite' }}></div>
            ...
            <section className="w-full md:w-[40%] p-8 md:p-12 lg:p-16 flex flex-col justify-between relative overflow-hidden hidden md:flex" style={{ background: 'linear-gradient(to bottom, rgba(230,57,70,0.05), transparent)' }}>
              ...
              <div className="relative z-10 flex-1 flex flex-col justify-center">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#e63946]/20 bg-[#e63946]/10 backdrop-blur-sm mb-8 w-fit">
                  ...
                  <span className="text-sm font-semibold text-[#e63946] tracking-wide">اشتراك {selectedPlanForPay.name} {isAnnual ? 'السنوي' : 'الشهري'}</span>
                </div>
                
                <h2 className="text-white/60 text-base uppercase tracking-wider font-semibold mb-4">إجمالي المبلغ المستحق</h2>
                
                <div className="flex items-baseline gap-2 mb-8">
                  <span className="text-5xl lg:text-7xl font-bold font-sans tracking-tight" style={{ background: 'linear-gradient(to right, #fff, #a58a8a)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                ...
                <div className="space-y-4 mt-4">
                  {(selectedPlanForPay.features || ['وصول غير محدود لجميع النماذج', 'أولوية في أوقات الذروة', 'دعم فني مخصص 24/7']).slice(0, 4).map((feature: string, idx: number) => (
                    <div key={idx} className="flex items-center gap-3 text-sm text-white/80">
                      <Check className="w-5 h-5 text-[#e63946] flex-shrink-0" />
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
