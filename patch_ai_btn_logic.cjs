...
const newNewNoteAI = `                  onClick={() => {
                    ...
                    setEditIsPinned(newNote.isPinned);
                    setEditTags(newNote.tags || []);
                    setEditImageUrl(newNote.imageUrl || '');
                    setEditAudioUrl(newNote.audioUrl || '');
                    setEditTodoItems(newNote.items || []);
...
code = code.replace(oldNewNoteAI, newNewNoteAI);

const oldEditNoteAI = `                  onClick={() => {
...
code = code.replace(oldEditNoteAI, newEditNoteAI);
fs.writeFileSync('src/components/KeepNotes.tsx', code);
console.log("KeepNotes logic updated!");
