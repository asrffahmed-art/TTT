...
// Two views in one tab: المنبهات (alarms) and التقويم (calendar).

import { useState, useEffect } from 'react';
import { AlarmClock, CalendarDays, Plus, Trash2, ChevronRight, ChevronLeft, Repeat2, BellRing, CalendarPlus, Sparkles } from 'lucide-react';
...
export function StudyTools() {
  ...
  const [view, setView] = useState<'alarms' | 'calendar'>('alarms');
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" dir={isAr ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className="shrink-0 px-4 pt-5 pb-3 bg-gradient-to-b from-white/5 to-transparent border-b border-white/5">
        <div className="max-w-xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shadow-md ${theme.activeTabClass}`}>
              <Sparkles className={`w-5 h-5 ${theme.textAccent}`} />
            </div>
            <div>
              <h1 className="text-lg font-black text-white">{isAr ? 'أدوات الدراسة' : 'Study Tools'}</h1>
              <p className="text-[11px] text-white/50">{isAr ? 'منبه وتقويم دراسي — يعمل محليًا على جهازك' : 'Alarms & study calendar — runs locally on your device'}</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer ${view === 'alarms' ? theme.activeTabClass : 'bg-white/5 text-white/50 border border-white/10 hover:bg-white/10'}`}
            >
              <AlarmClock className="w-4 h-4" />
              <span>{isAr ? 'المنبهات' : 'Alarms'}</span>
            </button>
            <button
              onClick={() => setView('calendar')}
              className={`flex-1 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer ${view === 'calendar' ? theme.activeTabClass : 'bg-white/5 text-white/50 border border-white/10 hover:bg-white/10'}`}
            >
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        <span>{dayNum}</span>
                        {marked && (
                          <span className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white/90' : 'bg-pink-400'}`} />
                        )}
                      </button>
                    );
                  })}
              ...
              <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-black text-white">
                    {isAr ? 'أحداث يوم' : 'Events on'} {selectedDay}
                  </h3>
                  <span className="text-[10px] text-white/40">{selectedEvents.length + selectedOnceReminders.length}</span>
                </div>

                {selectedEvents.length === 0 && selectedOnceReminders.length === 0 && (
