...
export function AdminAudioDiagnostics() {
  ...
  const runEnvDiagnostics = (ctx: AudioContext) => {
    addLog('ENV', `AudioContext state: ${ctx.state}`);
    addLog('ENV', `AudioContext sampleRate: ${ctx.sampleRate}`);
    addLog('ENV', `AudioContext baseLatency: ${(ctx as any).baseLatency}`);
    addLog('ENV', `AudioContext outputLatency: ${(ctx as any).outputLatency}`);
  ...
  const testOscillator = () => {
    ...
    try {
      ...
      addLog('OSCILLATOR', `ERROR = ${e?.message || e}`);
  ...
  const testAudioBuffer = () => {
    ...
    try {
      ...
      addLog('BUFFER', `ERROR = ${e?.message || e}`);
  ...
  const testHTMLAudioElement = () => {
    ...
    try {
      ...
      audio.play().catch(e => {
        addLog('HTML_AUDIO', `ERROR = ${e?.message || e}`);
        addLog('HTML_AUDIO', 'PASS/FAIL = FAIL');
      });
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'gemini_connected') {
        ...
        try {
          ...
          addLog('PLAYBACK', `PCM DECODE = FAIL (${e?.message || e})`);
        }
      }
    };
  };
  ...
  const testMicrophone = async () => {
    ...
    try {
      ...
      addLog('MICROPHONE', `Permission/Error = FAIL (${e?.message || e})`);
    }
  };
  ...
  const testFullNativeAudio = () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
      ...
      const ws = new WebSocket(`${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/api/live-audio?model=gemini-3.1-flash-live-preview&voice=Puck`);
      ...
      addLog('FULL_TEST', `MICROPHONE = FAIL (${e?.message || e})`);
    });
  };
