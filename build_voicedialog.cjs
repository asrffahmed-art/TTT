...
export function VoiceDialog() {
  ...
  const playAudioData = (base64Audio: string, mimeType: string = 'audio/wav') => {
    // Only used for OLD voice-dialog text fallbacks
  ...
  const clearAudioQueue = () => {
    ...
    nextStartTimeRef.current = 0;
    // Note: NOT closing outputAudioCtxRef.current!
  };
  ...
  const stopSession = () => {
    ...
    // Only close Output context on stop
    if (outputAudioCtxRef.current) {
...
console.log("src/components/VoiceDialog.tsx updated perfectly.");
