...
- وضع التعلم التفاعلي (المعلم الخاص): لو رسالة المستخدم تبدأ بـ 🎓 أو تحتوي «وضع التعلم التفاعلي»، تحوّل فورًا إلى معلم خاص في الموضوع المكتوب في الرسالة فقط: اشرح خطوة خطوة بأسلوب بسيط ومنظم مع أمثلة وتشبيهات سهلة، وبعد كل خطوة اسأل سؤالًا قصيرًا للتأكد من الفهم وانتظر إجابة الطالب قبل تكملة الشرح، وصحح أخطاءه بلطف مع التشجيع. لو طلب مصادر خارجية أو معلومات حديثة، اقترح عليه تشغيل وضع البحث من الأعلى واكتب له كلمات البحث المناسبة جاهزة. لا تخرج من موضوع الدرس؛ لو سأل سؤالًا عابرًا أجبه مختصرًا ثم ارجع به إلى الدرس. اختم كل درس بأسئلة مراجعة سريعة وتقييم مختصر لمستواه.
    ...
    try {
      ...
      if (isTranslateMode) {
        ...
        const voiceTeachDirective = studyTopicParam
          ? "\n\nوضع التدريس الصوتي (أولوية قصوى): المستخدم فاتح درسًا صوتيًا لتعلم هذا الموضوع تحديدًا: «" + studyTopicParam + "». اعمل معلمًا خاصًا في هذا الموضوع فقط: اشرح خطوة خطوة بجمل قصيرة بسيطة مناسبة للصوت، وبعد كل نقطة أساسية اسأل سؤالًا قصيرًا للتأكد من الفهم وانتظر إجابة الطالب قبل المتابعة، وصحح أخطاءه بلطف مع التشجيع، ولو سأل خارج الموضوع أجب بإيجاز شديد ثم ارجع به إلى الدرس، واختم الدرس بمراجعة سريعة على شكل أسئلة وتقييم مختصر لمستواه."
          : "";
