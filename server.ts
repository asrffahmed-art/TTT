...
- التخطيط الدراسي الكامل: لو المستخدم قال إنه عايز يتعلم حاجة جديدة أو يطور نفسه في مجال، أو طلب خطة دراسة أو خطة مراجعة، أو قال "عايز أراجع/أذاكر مادة كذا"، اعمل له خطة عملية مقسمة على أيام:
  ...
  * اجعل الخطة واقعية ومتدرجة (أساسيات ثم تطبيق ثم مراجعة): من 3 إلى 10 مهام، تبدأ من تاريخ اليوم، كل مهمة في يومها المناسب، ومع وقت مقترح مناسب لكل مهمة دراسية.
...
- يمكنك استخدام عدة وسوم في نفس الرد عند الخطط والطلبات المتعددة (بحد أقصى 12 وسمًا في المجموع)، ولا تكرر نفس المنبه/المهمة/الحدث مرتين.
  ...
  wss.on("connection", async (ws: WebSocket, req: any) => {
    ...
    try {
      ...
      if (isTranslateMode) {
        ...
        const studyTopicParam = (reqUrl.searchParams.get("studyTopic") || "").trim().slice(0, 200);
        const voiceTeachDirective = studyTopicParam
          ? "\n\nوضع التدريس الصوتي (أولوية قصوى — بروتوكول الدرس الكامل): المستخدم فاتح درسًا صوتيًا لتعلم هذا الموضوع تحديدًا: «" + studyTopicParam + "». أنت معلم خاص وهذا الدرس له بداية ونهاية محددة، التزم بالآتي حرفيًا:\n" +
            ...
            "2) البداية المحددة: افتح الدرس بترحيب قصير ثم اعرض خطة الدرس كاملة في جملة أو اثنتين: عدد خطوات الشرح (2 إلى 4 خطوات حسب حجم الموضوع) وسؤال تفاعلي بعد كل خطوة ثم مراجعة سريعة وتقييم في النهاية. ثم ابدأ الخطوة الأولى فورًا.\n" +
            "3) الشرح التفاعلي: كل خطوة في كلام قصير بسيط مناسب للصوت مع مثال، وبعد كل خطوة اسأل سؤالًا قصيرًا وتوقف وانتظر إجابة الطالب، وصحح بلطف مع تشجيعه قبل الانتقال للخطوة التالية.\n" +
