...
const targetStr = `  wss.on("connection", async (ws: WebSocket, req) => {
    try {
      if (!ai) { 
...
const replacementStr = `  wss.on("connection", async (ws: WebSocket, req: any) => {
    try {
      if (!ai) { 
         await refreshAiClient();
      }
...
if (code.includes(targetStr)) {
  ...
  console.log("Exact match not found, inspecting exact lines...");
  const idx = code.indexOf('wss.on("connection"');
  console.log("Index of wss.on connection:", idx);
  if (idx !== -1) {
    console.log("Substring:", JSON.stringify(code.slice(idx, idx + 400)));
  }
}
